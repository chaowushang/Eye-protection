...
### v2.1 (当前版本)
*   **开关逻辑重构**：彻底修复禁用站点偶发失效问题，确保黑名单严格执行。
*   **菜单净化交互**：精简选项为“启用/禁用”，支持状态动态互斥显示，消除冗余。
*   **性能极致优化**：收窄扫描范围至核心容器标签，优化正则解析效率，大幅降低 CPU 占用。
*   **锁定稳定性增强**：改进背景色识别与锁定机制，有效减少复杂页面（SPA）的视觉闪烁。
...
### v1.1
*   **算法优化**：改进颜色识别逻辑，解决部分网页文字对比度过低的问题。
*   **加载加速**：更换国内 CDN 链接，提升脚本加载与更新速度。
*   **站点记忆**：支持单站点“启用/禁用”开关功能，自主控制护眼范围。

### v1.0
*   **基础护眼**：提供乡土黄、豆沙绿、浅色灰、淡橄榄四种经典护眼配色。
*   **偏好记忆**：自动保存用户的背景色选择，跨页面同步生效。
*   **全网适配**：实现基础网页背景颜色的自动化智能替换。
